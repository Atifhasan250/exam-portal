...
const TRANSCRIPT_FETCH_TIMEOUT_MS = 10000
const TRANSCRIPT_TEXT_MAX_LENGTH = 200000
...
export async function previewYouTubeVideo(urlOrId, preferredLanguage = '') {
  ...
  return enrichVideoWithTranscript(video, preferredLanguage)
}
...
async function enrichVideoWithTranscript(video, preferredLanguage) {
  const transcript = await fetchTranscriptText(video.url, preferredLanguage).catch((error) => ({
    text: '',
    language: '',
...
async function fetchTranscriptText(videoUrl, preferredLanguage) {
  const preferredLanguages = ['bn', 'en', 'hi'].includes(preferredLanguage) ? [preferredLanguage] : []
  const attempts = [...preferredLanguages, '']
  let lastError = null

  for (const lang of attempts) {
    try {
      const items = await fetchTranscriptItems(videoUrl, lang)
      const text = normalizeTranscriptItems(items)
      if (text) return { text, language: items.find((item) => item.lang)?.lang || lang || '' }
    } catch (error) {
      lastError = error
    }
  }
...
async function fetchTranscriptItems(videoUrl, lang) {
  const controller = new AbortController()
  const timeoutId = abortAfter(controller, TRANSCRIPT_FETCH_TIMEOUT_MS, 'YouTube transcript request timed out')

  try {
    return await fetchYouTubeTranscript(videoUrl, {
      ...(lang ? { lang } : {}),
      fetch: (url, options = {}) => fetch(url, { ...options, signal: controller.signal }),
    })
  } catch (error) {
    if (controller.signal.aborted) {
      throw new Error(controller.signal.reason?.message || 'YouTube transcript request timed out', { cause: error })
    }
    throw error
  } finally {
    clearTimeout(timeoutId)
...
function normalizeTranscriptItems(items) {
  return (items || [])
    ...
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, TRANSCRIPT_TEXT_MAX_LENGTH)
}
