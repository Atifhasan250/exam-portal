...
export async function getRankedLiveSubmissionPage(filter, { limit = 50, offset = 0 } = {}) {
  ...
  const query = normalizeSubmissionAggregateFilter({ ...filter, wasLive: true })
    ...
    { $sort: { score: -1, submittedAt: 1, _id: 1 } },
    {
      $group: {
        _id: {
          ...
          userId: {
            $cond: [
              { $gt: [{ $strLenCP: { $ifNull: ['$clerkUserId', ''] } }, 0] },
              '$clerkUserId',
              { $toString: '$_id' },
            ],
          },
  ...
  return {
    ...
    nextOffset: normalizedOffset + submissions.length,
    hasMore: normalizedOffset + submissions.length < totalCount,
  }
}

function normalizeSubmissionAggregateFilter(filter) {
  const normalized = { ...filter }
  if (typeof normalized.examId === 'string' && mongoose.Types.ObjectId.isValid(normalized.examId)) {
    normalized.examId = new mongoose.Types.ObjectId(normalized.examId)
  } else if (normalized.examId?.$in) {
    normalized.examId = {
      ...normalized.examId,
      $in: normalized.examId.$in.map((id) => (
