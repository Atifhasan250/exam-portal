...
import ResourceAiChatHistory from '@/lib/models/ResourceAiChatHistory'
...
export async function getCachedAiAnswer(resourceId, message, resourceUpdatedAt) {
  const key = responseCacheKey(resourceId, message, resourceUpdatedAt)
...
export async function setCachedAiAnswer(resourceId, message, resourceUpdatedAt, value) {
  const key = responseCacheKey(resourceId, message, resourceUpdatedAt)
...
export async function getAiChatHistory(userId, resourceId) {
  ...
  try {
    if (redis) {
      const value = await redis.get(key)
      if (!value) return await getMongoAiChatHistory(key) || []
      const parsed = typeof value === 'string' ? JSON.parse(value) : value
      const messages = sanitizeHistory(parsed)
      return messages
    }
    ...
    if (!entry || entry.expiresAt <= Date.now()) {
      ...
      return await getMongoAiChatHistory(key) || []
    }
    const messages = sanitizeHistory(entry.value)
    return messages
  } catch (error) {
...
export async function appendAiChatHistory(userId, resourceId, newMessages) {
  ...
  return setAiChatHistory(userId, resourceId, key, messages)
}
...
export async function mergeAiContinuationHistory(userId, resourceId, assistantText) {
  ...
  return setAiChatHistory(userId, resourceId, key, messages.slice(-40))
}

export async function deleteAiUserData(userId, options = {}) {
  if (!userId) return { deletedCount: 0 }
  const userHash = hash(userId)
  const query = { userHash }
  const operation = ResourceAiChatHistory.deleteMany(query)
  const result = options.session ? await operation.session(options.session) : await operation
  if (options.clearRedis !== false) {
    await deleteAiUserRedisHistory(userHash)
  }
  return { deletedCount: result.deletedCount || 0 }
}

async function deleteAiUserRedisHistory(userHash) {
  const redis = getAiCacheRedisClient()
  if (!redis) {
    for (const key of memoryHistory.keys()) {
      if (key.startsWith(`${AI_PREFIX}:history:${userHash}:`)) {
        memoryHistory.delete(key)
      }
    }
    return
  ...
  const match = `${AI_PREFIX}:history:${userHash}:*`
  let cursor = '0'
  ...
  try {
    ...
    logger.warn('[resource-ai] redis history cleanup failed', { error })
  }
}

async function setAiChatHistory(userId, resourceId, key, messages) {
  ...
  await setMongoAiChatHistory(userId, resourceId, key, messages)
  return messages
}

async function getMongoAiChatHistory(key) {
  if (mongoose.connection.readyState !== 1) return null

  try {
    const doc = await ResourceAiChatHistory.findOne({
      key,
      expiresAt: { $gt: new Date() },
    }).lean()
    if (!doc) return null
    return sanitizeHistory(doc.messages)
  } catch (error) {
    logger.warn('[resource-ai] mongo history read failed', { error })
    return null
  }
}

async function setMongoAiChatHistory(userId, resourceId, key, messages) {
  if (mongoose.connection.readyState !== 1) return

  const { historyTtlSeconds } = getAiLimits()
  const expiresAt = new Date(Date.now() + historyTtlSeconds * 1000)
  const sanitizedMessages = sanitizeHistory(messages)

  try {
    await ResourceAiChatHistory.findOneAndUpdate(
      { key },
      {
        $set: {
          key,
          userHash: hash(userId),
          resourceHash: hash(resourceId),
          resourceId,
    ...
    logger.warn('[resource-ai] mongo history write failed', { error })
  }
}
...
export async function generateResourceAiAnswer(resource, message, history = []) {
  ...
  for (const provider of providers) {
    try {
      ...
      errors.push({ provider: provider.name, envKey: provider.envKey, model: provider.model, message: error.message })
      logger.warn('[resource-ai] provider failed', {
...
function getAiRedisClient() {
  if (!process.env.UPSTASH_REDIS_REST_URL || !process.env.UPSTASH_REDIS_REST_TOKEN) {
    if (globalForAi.irzAiCacheRedisClient) return globalForAi.irzAiCacheRedisClient
    if (!process.env.UPSTASH_REDIS_CACHE_REST_URL || !process.env.UPSTASH_REDIS_CACHE_REST_TOKEN) return null
    ...
    globalForAi.irzAiCacheRedisClient = globalForAi.irzAiRedisClient
...
function responseCacheKey(resourceId, message, resourceUpdatedAt) {
  return `${AI_PREFIX}:answer:${hash(resourceId)}:${hash(resourceUpdatedAt || '')}:${hash(normalizeAiMessage(message).toLowerCase())}`
}
...
function getOpenRouterProviders() {
  return [1, 2, 3, 4, 5].map((index) => {
    ...
    return {
      ...
      envKey: `OPENROUTER_API_KEY_${index}`,
      apiKey,
      model,
    }
  }).filter(Boolean)
