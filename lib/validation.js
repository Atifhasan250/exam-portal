...
const optionalUrlSchema = z
  ...
  .trim()
  .max(1200)
  .refine(safeResourceUrl, 'URL must be a valid http or https URL')
  .optional()
  .or(z.literal(''))
...
export const imageKitAssetSchema = z.object({
  ...
  size: z.coerce.number().int().min(1).max(1024 * 1024 * 1024),
  ...
  thumbnailUrl: optionalUrlSchema,
  folder: z.string().trim().max(260).default('/resources'),
})
