...
export const MAX_EXAM_QUESTIONS = 300
export const MAX_PLANNER_HISTORY_DAYS = 400
export const MAX_HABIT_KEYS_PER_DAY = 30
...
export const submitExamSchema = z.object({
  ...
  practiceAttemptId: z
    .string()
    .regex(/^[0-9a-fA-F]{24}$/, 'Invalid practice attempt id')
    .optional(),
  answers: z
    .record(z.string(), z.number().int().min(0).max(4))
    .refine(
      (answers) => Object.keys(answers).length <= MAX_EXAM_QUESTIONS,
...
export const attemptAnswerSchema = z.object({
  questionIndex: z.number().int().min(0).max(MAX_EXAM_QUESTIONS - 1),
  optionIndex: z.number().int().min(0).max(4),
})

export const attemptEventSchema = z.object({
  type: z.enum(['visibility-hidden', 'beforeunload', 'browser-back', 'manual-submit', 'timer-expired']),
...
export const addQuestionsSchema = z.object({
  questions: z
    ...
    .max(MAX_EXAM_QUESTIONS, `At most ${MAX_EXAM_QUESTIONS} questions can be added at once`),
})

export const resourceQuizQuestionsSchema = z
  .array(questionSchema.extend({
    order: z.coerce.number().int().min(0).max(MAX_EXAM_QUESTIONS).optional(),
  }))
  .max(MAX_EXAM_QUESTIONS, `At most ${MAX_EXAM_QUESTIONS} quiz questions can be saved for a resource`)
  .default([])
...
export const reorderQuestionsSchema = z.object({
  orderedIds: z
    ...
    .max(MAX_EXAM_QUESTIONS, 'Too many questions to reorder'),
})
...
function imageKitUrlMatchesConfiguredHost(value) {
  if (!value) return true
  ...
  try {
    ...
    const configuredHost = endpoint ? new URL(endpoint).hostname : ''
    if (configuredHost) return url.hostname === configuredHost
    return url.hostname === 'ik.imagekit.io' || url.hostname.endsWith('.imagekit.io')
  } catch {
    return false
  }
}
...
export function normalizeHabitHistory(history = {}) {
  const entries = Object.entries(history)
    ...
    .slice(-MAX_PLANNER_HISTORY_DAYS)

  return Object.fromEntries(entries.map(([date, day]) => [
    date,
    Object.fromEntries(
      Object.entries(day)
        .filter(([key, value]) => key.length <= 80 && typeof value === 'boolean')
        .slice(0, MAX_HABIT_KEYS_PER_DAY),
    ),
  ]))
}
...
const habitHistorySchema = z
  .record(
    ...
    z.record(z.string().max(80), z.boolean())
      .refine(
        (day) => Object.keys(day).length <= MAX_HABIT_KEYS_PER_DAY,
        { message: `A day can track at most ${MAX_HABIT_KEYS_PER_DAY} habits` },
      ),
  )
  .refine(
    (history) => Object.keys(history).length <= MAX_PLANNER_HISTORY_DAYS,
    { message: `Habit history can include at most ${MAX_PLANNER_HISTORY_DAYS} days` },
  )
  .transform(normalizeHabitHistory)
  .default({})
...
export const createResourceCategorySchema = z.object({
  ...
  color: z.string().trim().max(40).default('#ea7a53'),
...
const resourceBaseSchema = z.object({
  ...
  transcriptText: z.string().trim().max(200000).default(''),
  ...
  imagekitUrl: optionalUrlSchema.refine(imageKitUrlMatchesConfiguredHost, 'ImageKit URL must use the configured ImageKit host'),
  ...
  language: z.enum(['bn', 'en', 'hi', 'mixed', 'other']).default('bn'),
  tags: tagListSchema,
  topicTags: tagListSchema,
  quizQuestions: resourceQuizQuestionsSchema,
  order: z.coerce.number().int().min(0).max(100000).default(0),
...
function validateResourceTypeFields(data, ctx) {
...
export const createResourceSchema = resourceBaseSchema.superRefine(validateResourceTypeFields)

export const updateResourceSchema = resourceBaseSchema.partial().superRefine((data, ctx) => {
  if (!data.type) return
  validateResourceTypeFields(data, ctx)
})
...
export const youtubePreviewSchema = z.object({
  ...
  language: z.enum(['bn', 'en', 'hi', 'mixed', 'other']).optional(),
})

export const youtubePlaylistPreviewSchema = z.object({
  url: z.string().trim().min(1).max(1200),
  limit: z.coerce.number().int().min(1).max(50).default(50),
  pageToken: z.string().trim().max(300).optional().or(z.literal('')),
})
...
export const imageKitAssetSchema = z.object({
  ...
  imagekitUrl: z.string().trim().min(1).max(1200)
    .refine(safeResourceUrl, 'ImageKit URL must be valid')
    .refine(imageKitUrlMatchesConfiguredHost, 'ImageKit URL must use the configured ImageKit host'),
  thumbnailUrl: optionalUrlSchema,
  folder: z.string().trim().max(260).default('/resources').refine(
    (value) => (
      (value === '/resources' || value.startsWith('/resources/')) &&
      !value.includes('..') &&
      !value.includes('\\')
    ),
    'Uploads must stay under /resources without traversal or backslashes',
  ),
})
...
const notificationPathSchema = z
  ...
  .refine(
    ...
    'Open path must be an internal app path',
  )
...
export const adminNotificationSchema = z.object({
  title: z
    ...
    .max(80, 'Notification title must be 80 characters or fewer'),
  body: z
    .string()
    .trim()
    .min(1, 'Notification description is required')
    .max(240, 'Notification description must be 240 characters or fewer'),
  url: notificationPathSchema,
  scheduledAt: z
    .string()
    .datetime({ offset: true })
    .optional()
