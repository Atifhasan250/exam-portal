...
function resourceCacheKey(key) {
  return `resources:${RESOURCE_CACHE_VERSION}:${key}`
}

function resourceItemCacheKey(key) {
  return `resource:${RESOURCE_CACHE_VERSION}:${key}`
}
...
export async function getCachedPublishedExamPage(params = {}) {
  ...
  return getCachedJson(`exams:page:${stableKeyPart(normalized)}`, async () => {
    await connectDB()
    const { query, sort } = buildExamPageQuery(normalized.status)
...
export async function getCachedInitialResourcePageData() {
  return getCachedJson(resourceCacheKey('home'), async () => {
    await connectDB()
...
export async function getCachedInitialCategoryPageData(slug) {
  return getCachedJson(resourceCacheKey(`category-page:${slug}`), async () => {
    try {
      await connectDB()
...
export async function getCachedCategoryBySlug(slug) {
  return getCachedJson(resourceCacheKey(`category:${slug}`), async () => {
    try {
      await connectDB()
...
export async function getCachedPublicResources(params) {
  ...
  return getCachedJson(resourceCacheKey(`list:${stableKeyPart(normalized)}`), async () => {
    await connectDB()
    ...
    if (isSearch) query.$text = { $search: normalized.q }
    ...
    let totalCount
    try {
      ;({ resources, totalCount } = await findPublicResourcesPage(query, projection, sortSpec, normalized))
    } catch (error) {
      if (!isSearch || !isMissingTextIndexError(error)) throw error
      ...
      ;({ resources, totalCount } = await findPublicResourcesPage(
        query,
        undefined,
        normalized.sort === 'latest' ? { createdAt: -1 } : { order: 1, createdAt: -1 },
        normalized,
      ))
    }
...
function isMissingTextIndexError(error) {
  return (
    error?.code === 27 ||
    /text index required|no text index/i.test(error?.message || '')
  )
}

export async function getCachedResourceBySlug(slug) {
  return getCachedJson(resourceItemCacheKey(`watch:${slug}`), async () => {
    await connectDB()
...
export async function getCachedResourcePageBySlug(slug) {
  return getCachedJson(resourceItemCacheKey(`view:${slug}`), async () => {
    await connectDB()
    const idCandidate = slug.split('-').at(-1)
    const query = {
      published: true,
...
export async function getCachedSiblingResources(resource) {
  ...
  return getCachedJson(resourceItemCacheKey(`siblings:${resourceId}`), async () => {
    await connectDB()
...
export async function getCachedCategorySiblingResources(resource) {
  ...
  const resourceId = resource._id?.toString()
  return getCachedJson(resourceItemCacheKey(`category-siblings:${resourceId}`), async () => {
    await connectDB()
    const resources = await Resource.find(
...
function buildExamPageQuery(status) {
  const now = new Date()
  if (status === 'live') {
    return {
      query: { published: true, liveStart: { $lte: now }, liveEnd: { $gte: now } },
      sort: { liveEnd: 1, _id: 1 },
    }
  }
  if (status === 'upcoming') {
    return {
      query: { published: true, liveStart: { $gt: now } },
      sort: { liveStart: 1, _id: 1 },
    }
  }
  return {
    query: { published: true, liveEnd: { $lt: now } },
    sort: { liveEnd: -1, _id: 1 },
  }
}
