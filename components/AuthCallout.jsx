...
export default function AuthCallout({ title, description, href = '/sign-in', action = 'Sign In' }) {
  return (
    <div className="bg-theme-surface border border-theme-border rounded-2xl p-8 shadow-sm text-center max-w-lg mx-auto">
      <div className="w-16 h-16 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-theme-accent mx-auto mb-4">
      ...
      <Link
        href={href}
        className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-theme-accent text-white font-bold hover:opacity-90 transition-all"
      >
        {action}
      </Link>
    </div>
