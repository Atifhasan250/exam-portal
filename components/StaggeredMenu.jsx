...
export const StaggeredMenu = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (socialTitle || socialLinks.length) {
      ...
      if (socialLinks.length) tl.to(socialLinks, { y: 0, opacity: 1, duration: 0.55, ease: 'power3.out', stagger: { each: 0.08 } }, sStart + 0.04)
    }
  ...
  const portal = (
    ...
    >
      <div className="sm-backdrop" onClick={closeOnClickAway ? closeMenu : undefined} />
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {layerColors.map((c, i) => (
          <div key={i} className="sm-prelayer" style={{ background: c }} />
        ))}
      </div>
      <aside ref={panelRef} className="staggered-menu-panel relative" aria-hidden={!open} id="staggered-menu-panel">
        {/* Dedicated Close Button inside the panel */}
        <button
          onClick={closeMenu}
          className="absolute top-6 right-6 sm:top-8 sm:right-8 w-12 h-12 rounded-full flex items-center justify-center bg-theme-bg/80 backdrop-blur border border-theme-border text-theme-primary hover:text-theme-accent hover:border-theme-accent transition-all z-20 shadow-sm"
